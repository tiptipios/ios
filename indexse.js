...
var app = new Vue({
    ...
    validateApiKey() {
      if (this.apiKeyInput === apiKey) {
          this.isApiKeyValid = true;
          alert("Key API hợp lệ. Bạn có thể sử dụng các chức năng.");
      } else {
          this.isApiKeyValid = false;
          alert("Key API không hợp lệ. Vui lòng nhập key API đúng.");
      }
  },
    mounted() {
...
function RollS() {
      if (!this.isApiKeyValid) {
          alert("Vui lòng nhập key API hợp lệ trước.");
          return;
      }
